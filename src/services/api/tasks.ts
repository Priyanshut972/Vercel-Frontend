...
export const getTasksOnUserAPI = async () => {
  ...
  const response = await axios.get<Task[]>(
    "${import.meta.env.VITE_API_URL}/api/v1/tasks/user",
    {
      headers: {
...
export const createTaskAPI = async (data: {
  ...
  await axios.post("${import.meta.env.VITE_API_URL}/api/v1/tasks", formData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const updateTaskAPI = async (data: {
  ...
  await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/tasks/${taskId}`, formData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const deleteTaskAPI = async (data: {
  ...
  await axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/tasks/${taskId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
