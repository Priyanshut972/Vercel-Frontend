...
export const getTasksOnUserAPI = async () => {
  ...
  const response = await axios.get<Task[]>(
    "https://localhost:5000/api/v1/tasks/user",
    {
      headers: {
...
export const createTaskAPI = async (data: {
  ...
  await axios.post("https://localhost:5000/api/v1/tasks", formData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const updateTaskAPI = async (data: {
  ...
  await axios.put(`https://localhost:5000/api/v1/tasks/${taskId}`, formData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const deleteTaskAPI = async (data: {
  ...
  await axios.delete(`https://localhost:5000/api/v1/tasks/${taskId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
